...
def register():
    if request.method == 'POST':
        ...
        if not username:
            error = "Sorry, you're going to need a username."
        elif not password:
            error = "Sorry, you're going to need a password."
        elif db.execute(
            'SELECT id FROM user WHERE username = ?', (username,)
        ).fetchone() is not None:
            error = "User {}? You're already registered. Welcome back.".format(username)
...
# A decorator can be used to check the user's login for each view.
